...
<div class="form-group col-md-12 @if($errors->has('hc_id')) has-error @endif">
    {!! Form::label('hc_id', 'Hộ chiếu ngoại giao:') !!}
    <div class="row">
        <div class="col-md-6">
            <select class="form-control select2 select2-hidden-accessible" name="hc_id">
                <option></option>
                @foreach($hcs as $hc)
                    <option @if (isset($xncHcNgoaiGiao) && $xncHcNgoaiGiao->hc_id == $hc->id) selected @endif value="{{$hc->id}}">
        ...
        <div class="col-md-6">
            <a href="{{route('hcNgoaiGiaos.create')}}">
...
@push('scripts')
    <script type="text/javascript">
        ...
        $('.select2').select2({
            placeholder: "Chọn hộ chiếu ngoại giao"
        })
    </script>
@endpush
